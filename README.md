...
- **🛠️ Инструменты (Tools)**: Локальное выполнение команд, работа с файлами — идеально для opencode, Continue и других AI-агентов
...
9. [🛠️ Инструменты (Tools)]#️-инструменты-tools)
10. [Python](#python)
11. [Доступные модели](#доступные-модели)
...
> ⚠️ **Экспериментальная функция.** Может быть нестабильной. API и поведение могут измениться. Используйте на свой риск.

Прокси поддерживает **function calling** — модель может выполнять действия на сервере: читать файлы, запускать команды, искать по коду. Это особенно полезно при использовании с **opencode**, **Continue**, **Cursor** и другими AI-агентами, которые полагаются на инструменты.
...
3. Модель отвечает JSON-вызовом инструмента
4. Прокси выполняет инструмент локально
5. Результаты возвращаются клиенту

> **Примечание:** Qwen API v2 не поддерживает multi-turn с tool results, поэтому результаты инструментов возвращаются напрямую, без повторного запроса к модели. Клиенты вроде opencode сами форматируют вывод.
...
[opencode](https://opencode.ai) — это CLI AI-агент, который использует tools для работы с кодовой базой. FreeQwenApi позволяет использовать opencode с бесплатным Qwen API.

**Настройка opencode:**

1. Запустите FreeQwenApi:
```bash
npm start
```

2. В настройках opencode укажите:
- **Base URL:** `http://localhost:3264/api`
- **API Key:** любой (или оставьте пустым)
- **Model:** `qwen-max-latest` или `qwen3-coder-plus`

3. Готово! Теперь opencode может:
   - Читать файлы проекта
   - Запускать команды (`git`, `npm`, `python`)
...
> **Важно:** Инструменты выполняют команды на сервере от имени пользователя, запустившего прокси.
> 
> - Не запускайте прокси с правами root/administrator
> - Будьте остороны с деструктивными командами (`rm`, `del`, `git reset --hard`)
